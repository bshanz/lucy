...
- **Memory + diary** - durable facts (`remember`) and timestamped moments (`log_moment`) in separate stores, both recallable conversationally ("what did I do last weekend?"). The newest memories are recalled into every owner turn automatically by an eve memory slot (`agent/memory.ts`), so a preference saved on Monday is simply there on Thursday; `recall_memories` searches further back and `forget_memory` retires a wrong one.
...
agent/
  ...
  memory.ts              # eve memory slot: recalls the newest facts into every owner turn
  channels/
  ...
  tools/                 # reminders, memory (remember/recall/forget), diary, gmail (send/reply/schedule),
